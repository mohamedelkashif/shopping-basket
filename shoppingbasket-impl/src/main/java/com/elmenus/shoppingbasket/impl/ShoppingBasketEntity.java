...
public class ShoppingBasketEntity
                ...
                evt -> shoppingBasketCreatedBehavior(new ShoppingBasketState(
                        Optional.of(
                                ShoppingBasket.builder()
                                        .id(evt.getId())
                                        .userUuid(evt.getUserUuid())
                                        .subTotal(evt.getSubTotal())
    ...
    private Behavior shoppingBasketCreatedBehavior(ShoppingBasketState state) {
        ...
        addAddItemInShoppingBasketBehavior(b, state);
    ...
    private void addAddItemInShoppingBasketBehavior(BehaviorBuilder b, ShoppingBasketState state) {
        b.setCommandHandler(ShoppingBasketCommand.AddItemInShoppingBasket.class,
                (cmd, ctx) -> {
                    int existingAmount = ShoppingBasketLogic.getAmountOfItems(state().getShoppingBasket().get(), cmd.getItemId());
                    if (existingAmount > 0) {
                        ...
                        state().getShoppingBasket().get().setSubTotal(state().getShoppingBasket().get().getSubTotal() + cmd.getPrice()*cmd.getInitialAmount());

                        // tax calculation, I assumed that tax is the 14% from the the price of al items in basket
                        double tax = state().getShoppingBasket().get().getSubTotal() * 0.14;

                        state().getShoppingBasket().get().setTax(Double.valueOf(df2.format(tax)));
                        state().getShoppingBasket().get().setTotal(state().getShoppingBasket().get().getSubTotal() + state().getShoppingBasket().get().getTax());
                        return ctx.thenPersist(
                                ShoppingBasketEvent.ItemAddedToShoppingBasket.builder()
                                        .shoppingBasketId(state().getShoppingBasket().get().getId())
                                        .userUuid(state().getShoppingBasket().get().getUserUuid())
