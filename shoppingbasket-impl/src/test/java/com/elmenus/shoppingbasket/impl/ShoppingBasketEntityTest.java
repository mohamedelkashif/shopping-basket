...
public class ShoppingBasketEntityTest {
    ...
    public void testCreateShoppingBasket() {
        ...
        outcome = driver.run(new ShoppingBasketCommand.CreateShoppingBasket(customerId));
    ...
    public void testAddItemInShoppingBasket() {
        ...
        driver.run(new ShoppingBasketCommand.CreateShoppingBasket(customerId));

        final String itemId = "12345_a";

        PersistentEntityTestDriver.Outcome<ShoppingBasketEvent, ShoppingBasketState> outcome;

        // Adding 0 items should fail
        outcome = driver.run(new ShoppingBasketCommand.AddItemInShoppingBasket(itemId, 0, 0));
        assertTrue("There are issues: " + outcome.issues(), outcome.issues().isEmpty());
        assertEquals("Should at least add one",
                ((PersistentEntity.InvalidCommandException)outcome.getReplies().get(0)).message());

        // Adding 2 items
        outcome = driver.run(new ShoppingBasketCommand.AddItemInShoppingBasket(itemId, 2,1));
        assertTrue("There are issues: " + outcome.issues(), outcome.issues().isEmpty());
        ...
        assertEquals(itemId, item.getUuid());
        assertEquals(2, item.getQuantity());

        // Adding the same product again should fail
        outcome = driver.run(new ShoppingBasketCommand.AddItemInShoppingBasket(itemId, 1,1));
        assertTrue("There are issues: " + outcome.issues(), outcome.issues().isEmpty());
    ...
    public void testAddUpdateItemAmountInShoppingBasket() {
        ...
        outcome = driver.run(new ShoppingBasketCommand.UpdateItemAmountInShoppingBasket(itemId, 2));
        assertTrue("There are issues: " + outcome.issues(), outcome.issues().isEmpty());
        assertEquals("Item has not yet been added, please add it first",
                ((PersistentEntity.InvalidCommandException)outcome.getReplies().get(0)).message());

        // Add the product
        driver.run(new ShoppingBasketCommand.AddItemInShoppingBasket(itemId, 1, 4));
    ...
    public void testRemoveItemFromShoppingBasket() {
        ...
        // Add the product
        driver.run(new ShoppingBasketCommand.AddItemInShoppingBasket(itemId, 1,5));
        ...
        // Remove the product
        outcome = driver.run(new ShoppingBasketCommand.RemoveItemFromShoppingBasket(itemId));
        assertTrue("There are issues: " + outcome.issues(), outcome.issues().isEmpty());
        assertEquals(Done.getInstance(), outcome.getReplies().get(0));
